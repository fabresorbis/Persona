...
function HomeBanner() {
    ...
    fetch("https://persona-backend-2fvi.onrender.com/api/home-content")
      ...
      .then(data => {
        if (data?.homeBanner?.imageUrl) {
          setCurrentBanner(data.homeBanner.imageUrl)
        }
      })
  }, [])
  ...
  const saveCrop = async () => {
    ...
    const res = await fetch("https://persona-backend-2fvi.onrender.com/api/home-content/home-banner", {
      method: "PUT",
      body: formData
    })
...
function DiscountBanner() {
    ...
    fetch("https://persona-backend-2fvi.onrender.com/api/home-content")
      ...
      .then(data => {
        if (data?.discountBanner) {
          ...
          setMessages(data.discountBanner.messages || [])
        }
      })
  }, [])
  ...
  const persist = async (nextEnabled, nextMessages) => {
    setSaving(true)
    await fetch("https://persona-backend-2fvi.onrender.com/api/home-content/discount-banner", {
      method: "PUT",
      ...
      <aside className="w-64 p-4 space-y-2 bg-white">
        <button
          ...
          className={`w-full px-3 py-2 rounded-lg text-left ${
            active === "homeBanner" ? "bg-black text-white" : "hover:bg-gray-100"
          }`}
        >
          Home Banner Image
        </button>
        ...
        <button
          ...
          className={`w-full px-3 py-2 rounded-lg text-left ${
            active === "discountBanner" ? "bg-black text-white" : "hover:bg-gray-100"
          }`}
        >
